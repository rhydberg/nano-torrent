...
func ParsePiece(index int, buf []byte, msg *Message) (int, error){
	...
	data := msg.Payload[8:]
	if begin+uint32(len(buf)) > uint32(len(buf)) {
...
func ParseHave(msg *Message) (int, error){
	if msg.ID != MsgHave {
		return -1, fmt.Errorf("expected HAVE (ID %d), got ID %d", MsgHave, msg.ID)
	}
	if len(msg.Payload) != 4 {
		return -1, fmt.Errorf("expected payload length 4, got length %d", len(msg.Payload))
	}
